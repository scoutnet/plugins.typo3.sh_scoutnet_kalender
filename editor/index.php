...
class SC_mod_user_scoutnet_kalender_editor_index extends t3lib_SCbase {
	...
	public function main() {
		...
		$markers['BEGIN_LABEL'] = 'Begin';
		$markers['END_LABEL'] = 'End';
		$markers['TITLE_LABEL'] = 'Title';
		$markers['ACTION_LABEL'] = 'Action';
		...
		foreach ($events as $event) {
			...
			$date_with_time = $start_date.(($start_time != '')?',&nbsp;'.$start_time:'').(($end_date.$end_time != '')?' bis ':'').($end_date != ''?$end_date:'').(($end_date.$end_time != '')?',&nbsp;':'').($end_time != ''?$end_time:'');
