...
class SC_mod_user_scoutnet_kalender_editor_index extends t3lib_SCbase {
	...
	public function main() {
		...
		if ($_GET['action'] == "edit") {
			...
			$markers['START_DATE_FIELD'] = $this->createDateInput('StartDate',date(),0);
	...
	private function createDateInput($name, $defaultValue, $value = ""){
		...
		return '<input type="hidden" name="'.$name.'" id="'.$name.'_value" value="'.$value.'">'.
'<select name="Start_Date[d]">
...
</select><select name="Start_Date[M]">
...
</select><select name="Start_Date[Y]">
